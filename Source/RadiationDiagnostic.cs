...
//                Debug.Log("T:" + i + ":" + codes[i].opcode + "::" + (codes[i].operand != null ? codes[i].operand.ToString() : codes[i].operand));
                ...
                {
                    codes.Insert( i + 5, codes[ radiationExposureLoad ].Clone()); // load 'COMPARE_RECOVERY_IMMEDIATE'
                    codes.Insert( i + 6, codes[ radiationExposureLoad + 1 ].Clone()); // load 'RadiationMonitor.Instance'
                    codes.Insert( i + 7, codes[ radiationExposureLoad + 2 ].Clone()); // load 'p2'
                    codes.Insert( i + 8, codes[ radiationExposureLoad + 3 ].Clone()); // call Invoke()
                    codes.Insert( i + 9, codes[ i + 4 ].Clone()); // if false
                    found = true;
                    break;
                }
            }
